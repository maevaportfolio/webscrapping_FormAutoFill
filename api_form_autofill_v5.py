...
FIELD_KEYWORDS = {
    ...
    'birth_day': ['day', 'jour', 'tag', 'birth_day', 'birthdate_day', 'day_of_birth', 'dd'],
    'birth_month': ['month', 'mois', 'monat', 'birth_month', 'birthdate_month', 'month_of_birth', 'mm'],
    'birth_year': ['year', 'année', 'an', 'jahr', 'birth_year', 'birthdate_year', 'year_of_birth', 'yyyy', 'yy'],
...
def get_field_type(field_name: str, threshold: float = 0.5) -> Optional[str]:
    """
    Détecte le type de champ basé sur le nom du champ.
    Retourne le type détecté (ex: 'gender', 'email', 'password') ou None
    """
    if not field_name:
        return None
    
    suggestions = detect_field_type_improved(field_name, use_levenshtein=True, threshold=threshold)
    if suggestions and len(suggestions) > 0:
        return suggestions[0]['field_type']  # Retourner le type le mieux matchant
...
def fill_forms(driver, provided_values: Dict = None, use_levenshtein: bool = True, threshold: float = 0.5, target_form_id: str = None) -> List[Dict]:
    ...
    for inp in all_inputs:
        try:
            ...
            if itype == 'radio':
                input_value = (inp.get_attribute('value') or '').lower().strip()
                ...
                # 1. Par nom exact du champ
                if name in values_lower:
                    target = values_lower[name]
                
                # 2. Par type de champ détecté (ex: 'gender')
                if target is None:
                    field_type = get_field_type(name or inp_id)
                    print(f"    [DEBUG] Radio '{name or inp_id}' -> Detected type: {field_type}")
                    if field_type and field_type in values_lower:
                        target = values_lower[field_type]
                        print(f"    [DEBUG] Radio '{name or inp_id}' -> Target value from type: {target}")
                
                # 3. Vérifier si ce radio correspond à la cible
                if target:
                    target_lower = str(target).lower().strip()
                    
                    # Synonymes pour gender
                    male_synonyms = ['male', 'm', 'man', 'homme', 'monsieur', 'mr', 'h']
                    female_synonyms = ['female', 'f', 'woman', 'femme', 'madame', 'mme', 'mlle', 'f']
                    
                    # Vérifier si le target et ce radio correspondent
                    matched = False
                    
                    # Si target est Male
                    if target_lower in male_synonyms:
                        # Vérifier si ce radio est Male
                        if any(syn in input_value for syn in male_synonyms) or any(syn in label_text for syn in male_synonyms):
                            matched = True
                            print(f"    [DEBUG] Radio matched MALE: input='{input_value}', label='{label_text}'")
                    
                    # Si target est Female
                    elif target_lower in female_synonyms:
                        # Vérifier si ce radio est Female
                        if any(syn in input_value for syn in female_synonyms) or any(syn in label_text for syn in female_synonyms):
                            matched = True
                            print(f"    [DEBUG] Radio matched FEMALE: input='{input_value}', label='{label_text}'")
                    
                    # Sinon: match exact
                    else:
                        if target_lower == input_value or target_lower == label_text:
                            matched = True
                            print(f"    [DEBUG] Radio matched EXACT: target='{target_lower}' vs input='{input_value}' or label='{label_text}'")
                    
                    if matched:
                        # Utiliser execute_script pour cocher le radio (plus fiable que click)
                        ...
                        print(f"  🔘 Radio '{field_id}' = '{label_text or input_value}' ✓")
                continue
            ...
            if itype == 'date':
                ...
                if value is not None:
                    try:
                        # Normaliser la date en format ISO 8601 (YYYY-MM-DD)
                        date_value = normalize_date_to_iso(str(value))
                        
                        if date_value:
                            # Utiliser execute_script pour setter le input type="date"
                            # C'est plus fiable que send_keys pour les champs HTML5
                            driver.execute_script("""
                            ...
                            print(f"  📅 DATE '{field_id}' = '{date_value}' ✓")
                        else:
                            print(f"  ⚠️  Format date invalide: {value}")
                    except Exception as e:
            ...
            # IMPORTANT: Tester UNIQUEMENT le field_id (pas le label pour éviter la confusion)
            detected_field_type = None
            ...
            # ÉTAPE 1: DEUX PASSES
            # PASS 1: Chercher les WORD MATCHES (le keyword est un mot complet du field_id)
            # Cela prioritise 'numero' dans 'adresse_numero' sur 'adresse'
            if detected_field_type is None:
                field_id_words = field_id.replace('-', '_').split('_')
                for keyword, ftype, length in all_keywords:
                    if keyword in field_id_words:  # WORD MATCH exact (word complet)
                        detected_field_type = ftype
                        detection_method = "WORD MATCH (field_id)"
                        confidence = 1.0
                        break
            
            # PASS 2: SUBSTRING MATCH (fallback si pas de word match)
            if detected_field_type is None:
                for keyword, ftype, length in all_keywords:
                    if keyword in field_id:
                        detected_field_type = ftype
                        detection_method = "SUBSTRING MATCH (field_id)"
                        confidence = 1.0
                        break
            
            # ÉTAPE 1b: Si pas trouvé via field_id, chercher via label (deux passes aussi)
            if detected_field_type is None and label_text:
                label_words = label_text.replace('-', '_').replace(' ', '_').split('_')
                for keyword, ftype, length in all_keywords:
                    if keyword in label_words:
                        detected_field_type = ftype
                        detection_method = "WORD MATCH (label)"
                        confidence = 1.0
                        break
            ...
            if detected_field_type is None and label_text:
                for keyword, ftype, length in all_keywords:
                    if keyword in label_text:
                        detected_field_type = ftype
                        detection_method = "SUBSTRING MATCH (label)"
                        confidence = 1.0
                        break
            ...
            if detected_field_type:
                ...
                if value:
                    print(f"      📌 ÉTAPE 3: Détection = '{detected_field_type}', Valeur trouvée")
            ...
            if value is None:
                if field_id in values_lower:
                    ...
                    if detection_method == "NONE":
                        ...
                        confidence = 1.0
                    print(f"      📌 ÉTAPE 4: Match EXACT par field_id, Valeur = '{value[:30]}...'")
                elif label_text and label_text in values_lower:
                    value = values_lower[label_text]
                    if detection_method == "NONE":
                        detection_method = "DIRECT LABEL"
                        confidence = 1.0
                    print(f"      📌 ÉTAPE 4: Match EXACT par label, Valeur = '{value[:30]}...'")
                else:
                    print(f"      ❌ ÉTAPE 4: Aucune valeur trouvée pour '{field_id}'")
